...
class Statistics:
    ...
    def GetPrecision(tp, fp):
        ...
        if divider == 0.0:
            ...
            precision = tp / (tp + fp)
        precision = precision#round(precision, 4)
        return precision
    ...
    @staticmethod
    def GetMCC(tp, fp, tn, fn):
        # exception on true non defective samples
        if tp == 0 and fp == 0 and fn == 0 and tn > 0: # positive sample, no prediction
            return 1.0

        if fn == 0 and fp > 0 and tp == 0: # false alarm
            return -1.0

        if tp == 0 and fp == 0 and fn > 0: # no positive detection, even when defect exist
            return -1.0

        top = float(tp * tn + fp * fn)
        bottom = np.sqrt(float((tp+fp)*(tp+fn)*(tn+fp)*(tn+fn)))
        if bottom == 0.0:
            g =1
        return top / bottom
